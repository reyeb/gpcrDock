...
#               raise raise AttributeError("Illegal operation: cannot set attribute")
    ...
    class Entry(object):
        ...
        def __init__(self, *args):
            ...
            self.seq = "".join(self.seq.split()) # remove all white space
        ...
        def toFastaString(self, remove_gaps=False):
  ...
  # BEGINNING OF ALI CLASS FUNCTIONS
    ...
    def copy_columns(self, selection):
        """copy_columns(selection) : copy columns in the selection to a new Ali object.
        
        accepts a list of booleans or a list of integer indeces.
        Assumes that all EntryGroups are aligned to each other."""
        
        # This is slow but does the job for now.
        # Could write more code to copy only the correct columns...
        #
        newali = copy.deepcopy(self)
        newali.keep_columns(selection)
        return newali
    ...
    def remove_gaps(self, template_seq=None):
        ...
        if template_seq:
          assert isinstance(template_seq[0], basestring)
...
class AlignmentMerger(object):
  def __init__(self, seq1, seq2, gaplist='-'):
    ...
    if seq1_aligned != seq2_aligned:
      ...
      from prosci.util.seq import align_identical
      seq1_aligned, seq2_aligned = align_identical(seq1, seq2)
