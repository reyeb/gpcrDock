...
residues['U2X'] = 'Y' # Modified Tyrosine with extra aromatic group linked to OH
residues['1OP'] = 'Y'
residues['U3X'] = 'F'
residues['TY5'] = 'Y'
residues['0A1'] = 'Y'
...
residues['UNX'] = 'X' # Unknown amino acid
residues['UNL'] = 'X' # Unknown amino acid
residues['XAA'] = 'X' # Unknown amino acid


def residueLetter(code):
  try:
    return residues[code.upper()]
  except KeyError:
    return 'X'

def residueCode(letter):
  try:
    return residues_inverted[letter.upper()]
  except KeyError:
    return 'XXX'

def residue_is_amino_acid(code):
  return code.upper() in residues


def is_different_residue(a, b):
  return (a.ires != b.ires) or (a.chain != b.chain) or (a.inscode != b.inscode)

def xresidue_boundaries(atoms):
  istart = 0
  iend   = 1  # slice syntax. self[i:i+1] returns a Pdb object containing a single atom.
  
  while istart < len(atoms):
    # search forward, for the end of the residue
    while iend < len(atoms) and not is_different_residue(atoms[iend-1], atoms[iend]):
      iend += 1
    yield (istart, iend)
    istart = iend
    iend += 1
...
def validate_pdb_code(pdbcode):
    import re
    p = re.compile('[<>&\|!]')
    assert isinstance(pdbcode, basestring) and len(pdbcode)==4, "Invalid PDB code: "+pdbcode
...
def intersectPdb(models):
  ...
  newmodels=[]
  for m in models:
    newmodel = Pdb(m.code, []) #newmodel = Pdb(m.code + "_subset", [])
    for start, end in m.xresidue_boundaries():
      atm = m[start]
...
class Pdb(object):
    ...
    def __init__(self, pdb, data=None, keepBefore1=True, deleteInsertions=False, nofilter=False, cryst1="", allowLigands=False, ligands=None):
        self.parent = None
        self.code   = ""
        self.data   = []
        self.allowLigands = allowLigands
        if allowLigands and not ligands:
          self.ligands = () # hetero atoms go into self.data
        else:
          self.ligands = Pdb([], allowLigands=True) # hetero atoms go into self.ligands
          self.ligands.append_atoms(ligands)
        self.cryst1 = cryst1
        ...
        if isinstance(pdb, Pdb):
          ...
          self.parent = pdb
          if data is not None:
            self.append_atoms(data)
            try:
              self.append_atoms(data.ligands)
            except AttributeError:
              pass
          else:
            self.append_atoms(pdb.data)
            self.append_atoms(pdb.ligands)
        elif isinstance(pdb, basestring):
            if "\n" in pdb:
              data = pdb
              pdb = ""
            if data is None:
        ...
        if data and not (self.data or self.ligands):
    ...
    def __getslice__(self, i, j):
        return Pdb(self, self.data[i:j], allowLigands=self.allowLigands)
    def __len__(self):
    ...
    def __str__(self):
        if self.code:
          return "TITLE     %-70s\n%s%s" % (self.code, self.cryst1, join("", self.data))
        else:
          return "%s%s" % (self.cryst1, join("", self.data))
    
    def __repr__(self):
        #return "{Pdb:%s:%s}" % (self.code, self.data.__str__())
        return 'Pdb("%s", %s, cryst1=%s, ligands=%s)' % (self.code, repr(self.data), repr(self.cryst1), repr(self.ligands))
    ...
    def parse_file(self, data):
        ...
        allowLigands = self.allowLigands
        temp_atoms = []
        atoms = []
        ligands = []
        chaincodes = {}
        ...
        for item in data:
          ...
          for line in item.splitlines():
            if line.startswith('CRYST1'):
              ...
              if atm.res in ("HOH", "WAT", "H2O"): # Ignore waters
                continue
              ...
              temp_atoms.append(atm)
              
              if not atm.hetatm:
                if atm.atom == "CA":
                  chaincodes[atm.chain] = chaincodes.get(atm.chain, 0) + 1
              
              must_renumber |= atm.iatom < 0
        
        from prosci.util.residue import Residue
        residues = [Residue(temp_atoms[start:end]) for start, end in xresidue_boundaries(temp_atoms)]
        for i in xrange(len(residues)):
          prevres = nextres = None
          if i > 0:
          ...
          is_connected = not (not prevres and not nextres) and (not prevres or prevres.is_left_neighbour_of(res)) and (not nextres or nextres.is_right_neighbour_of(res))
          
          make_atom = (not chaincodes or chaincodes.get(atm.chain, 0) > 1) and is_connected
          
          for atm in res:
            if atm.hetatm and atm.is_amino() and make_atom:
              #sys.stderr.write("WARNING: Pdb '%s' contains amino acids labelled as HETATM. Relablling as ATOM.\n"%self.code)
              atm.hetatm = False
            if allowLigands or not atm.hetatm:
              atoms.append(atm)
            else:
              ligands.append(atm)
        
        if not atoms and not ligands:
    ...
    def append_atoms(self, data):
        ...
        if data:
          ...
          allowLigands = self.allowLigands
          for atm in data:
            if atm.hetatm and not allowLigands:
              self.ligands.data.append(atm)
            else:
    ...
    def build_chainindex(self):
        ...
        for i,atom in enumerate(self):
            if (prev.chain != atom.chain) or (prev.ires > atom.ires):
    ...
    def is_residue_boundary(self, index):
        ...
        if index == 0 or index == len(self):
          return True
        return is_different_residue(self[index], self[index-1])
    ...
    def filter(self, negres=False, noca=True, inscode=False, altloc=True, duperes=True):
        ...
        for start,end in self.xresidue_boundaries():
          ...
          if noca:
            ok=False
            for i in xrange(start,end):
              if self[i].atom == "CA" or self[i].hetatm:
                ok=True
                break
            if not ok:
              continue
          ...
          if noca:
            ok=False
            for atm in resatms:
              if atm.atom == "CA" or self[i].hetatm:
                ok=True
                break
            if not ok:
              continue
    ...
    def deep_copy(self):
        ...
        p.code = self.code
        p.data = deepcopy(self.data)
        p.ligands = deepcopy(self.ligands)
        return p
