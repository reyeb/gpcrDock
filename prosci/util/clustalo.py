...
from prosci.util.gaps import isGap, gappify, deGappify
...
def reformat_seqs(seqs):
    ...
    if not isinstance(seqs, basestring):
      for i in xrange(len(seqs)):
        if not isinstance(seqs[i], basestring):
          assert len(seqs[i]) == 2
...
def align_unstable(seqs):
    ...
    p = subprocess.Popen("clustalo -i -", stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=sys.stderr, shell=True)
...
def reorder_alignment(alignment_txt, seqs_txt):
    ...
    assert (alignment_txt and seqs_txt) or not (alignment_txt or seqs_txt), str(alignment_txt)+"\n"+str(seqs_txt)
    ...
    for i in xrange(len(seqs)):
      try:
        ...
        print alignment_entries[seqs[i].code].seq
        print deGappify(alignment_entries[seqs[i].code].seq)
        print "-----"
        print seqs[i].seq
        print deGappify(seqs[i].seq)
        raise
